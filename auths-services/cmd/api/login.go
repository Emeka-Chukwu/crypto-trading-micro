...
func (s *Server) LoginUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		barf.Response(w).Status(http.StatusBadRequest).JSON(barf.Res{Status: false, Data: nil, Message: "error"})
		return
	}
	matched, err := s.repo.PasswordMatches(payload.Password, authModel.Password)
	if err != nil || !matched {
		barf.Response(w).Status(http.StatusUnauthorized).JSON(barf.Res{Status: false, Data: nil, Message: "error"})
		return
	}
	jwTMaker, err := data.NewJWTMaker(s.config.TokenSymmetricKey)
	if err != nil {
		barf.Response(w).Status(http.StatusInternalServerError).JSON(barf.Res{Status: false, Data: nil, Message: "error"})
		return
	}
